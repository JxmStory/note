...
//    @Transactional
     ...
     */
    @Transactional(transactionManager = "masterTransaction", rollbackFor = Exception.class)
//    @TransactionAppoint(value = {"masterTransaction", "slaveTransaction"})
    public Result updateUserMaster() {
        Result result = new Result();
        User user = userDao.getMaster(1);
        user.setUsername("abcdeft");
        result = userServiceInter.update(user);
        logger.info("【多数据源下事务测试】 updateUserMaster={}", result.toString());
        result = updateUserSlave(user);
        return result;
    }

    @Transactional(transactionManager = "slaveTransaction", propagation = Propagation.REQUIRES_NEW, rollbackFor = Exception.class)
//    @TransactionAppoint(value = {"masterTransaction", "slaveTransaction"})
    public Result updateUserSlave(User user) {
        Result result = new Result();
        result = userServiceInter.updateSlave(user);
        logger.info("【多数据源下事务测试】 updateUserSlave={}", result.toString());
        return result;
    }
