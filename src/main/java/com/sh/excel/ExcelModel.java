...
public class ExcelModel {
    ...
    public static void singleCascade(HttpServletResponse response) {
        // 创建一个excel
        @SuppressWarnings("resource")
        Workbook book = new XSSFWorkbook();

        // 创建需要用户填写的sheet
        XSSFSheet sheetPro = (XSSFSheet) book.createSheet("组织机构导入模板");
        sheetPro.setDefaultColumnWidth(16);
        Row row0 = sheetPro.createRow(0);
        row0.createCell(0).setCellValue("组织机构代码");
        row0.createCell(1).setCellValue("组织机构名称");
        row0.createCell(2).setCellValue("组织机构类型");
        row0.createCell(3).setCellValue("上级组织机构代码");
        row0.createCell(4).setCellValue("上级组织机构名称");
        row0.createCell(5).setCellValue("组织机构描述");

        //得到第一级省名称，放在列表里
        String[] departmentType = {"部门","院系","专业","班级","其他"};
//        String[] cityJiangSu = {"南京市","苏州市","盐城市"};

        //将有子区域的父区域放到一个数组中
        List<String[]> list = new ArrayList<>();
        list.add(departmentType);
//        list.add(cityJiangSu);
        ...
        setAD(sheetPro, departmentType, 2, 2);
//        setAD(sheetPro, cityJiangSu, 2, 2);
    ...
    public static void setAD(XSSFSheet sheetPro, String[] data, int firstCol, int lastCol) {
        ...
        CellRangeAddressList provRangeAddressList = new CellRangeAddressList(1, 20, firstCol, lastCol);
